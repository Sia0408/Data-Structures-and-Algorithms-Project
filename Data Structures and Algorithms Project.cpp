...
void bubbleSortByName(BookingRecord bookings[], int size)
{
    for(int i = 0; i < size - 1; i++)
    {
        for(int j = 0; j < size - i - 1; j++)
        {
            if(strcmp(bookings[j].customerName, bookings[j + 1].customerName) > 0)
            {
                BookingRecord temp = bookings[j];
                bookings[j] = bookings[j + 1];
                bookings[j + 1] = temp;
...
void sortBookings(char* currentUsername)
{
    ...
    cout << "\t\t========================================" << endl;
    cout << "\t\t1. Sort by Date (Oldest to Newest)" << endl;
    ...
    switch(sortChoice)
    {
        case 1:
            ...
            break;
        case 2:
            bubbleSortByName(bookings, count);
            cout << "\n\t\tBookings sorted by Name!" << endl;
            break;
        case 3:
            return;
        default:
...
void editBooking(char* currentUsername)
{
    ...
    if(foundIndex == -1)
    {
        cout << "\n\t\tBooking not found or you don't have permission!" << endl;
        cout << "\n\t\tPress any key to continue...";
        cin.ignore();
        cin.get();
        return;
...
int main()
{
	...
	do
	{
		do
		{
			...
			{
				cout<<"\n\t\tAdmin module - under development"<<endl;
				Sleep(3000);
			}
