...
void insertionSortByTableNumber(BookingRecord bookings[], int size)
{
    for(int i = 1; i < size; i++)
    {
        BookingRecord key = bookings[i];
        int j = i - 1;
        
        while(j >= 0 && bookings[j].tableNumber > key.tableNumber)
        {
...
int partitionByGuests(BookingRecord bookings[], int low, int high)
{
    int pivot = bookings[high].numberOfGuests;
    int i = low - 1;
    
    for(int j = low; j < high; j++)
    {
        if(bookings[j].numberOfGuests >= pivot)
        {
            i++;
            BookingRecord temp = bookings[i];
    ...
    BookingRecord temp = bookings[i + 1];
...
void quickSortByGuests(BookingRecord bookings[], int low, int high)
{
    if(low < high)
    {
        int pivotIndex = partitionByGuests(bookings, low, high);
        quickSortByGuests(bookings, low, pivotIndex - 1);
        quickSortByGuests(bookings, pivotIndex + 1, high);
    }
}
...
void sortBookings(char* currentUsername)
{
    ...
    cout << "\t\t3. Sort by Table Number (Ascending)" << endl;
    cout << "\t\t4. Sort by Number of Guests (Descending)" << endl;
    ...
    switch(sortChoice)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            insertionSortByTableNumber(bookings, count);
            cout << "\n\t\tBookings sorted by Table Number!" << endl;
            break;
        case 4:
            quickSortByGuests(bookings, 0, count - 1);
            cout << "\n\t\tBookings sorted by Number of Guests!" << endl;
            break;
        case 5:
            return;
        default:
